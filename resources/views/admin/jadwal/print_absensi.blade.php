...
<?php 
$mboh     = DB::table('jadwal')->where('JadwalID',$jadwal->JadwalID)->first();
$hari     = DB::table('hari')->where('HariID',$mboh->HariID)->first();
$prod     = str_replace(".", "", $mboh->ProdiID);
$prodi    = DB::table('prodi')->where('ProdiID',$prod)->first();
$mk       = DB::table('mk')->where('MKID',$mboh->MKID)->first();
$Matakulx = strtolower($mk->Nama);
$Matakul  = ucwords($Matakulx);

$ds       = DB::table('dosen')->where('Login',$mboh->DosenID)->first();
...
<table width='100%'>
<tr>
<td width='60%'>$mboh->TahunID</td>           
<td width='40%'>$hari->Nama</td> 
</tr>
...
<tr>
<td>$mboh->MKKode - $Matakul ($mboh->SKS SKS) - $mboh->NamaKelas  - Semester $mk->Sesi</td>             
<td>$mboh->RuangID</td> 
</tr>
...
echo "<table border='1' class='center' width='100%'>
  ...
  <th  style='text-align:center;width:50px'>16</th>
      ...
      foreach ($krs as $r){
        //$huruf= $r[GradeNilai];
        $NamaMhs 		= $r->NamaMhs;
        $NamaMhsx 	= strtolower($NamaMhs); //strtoupper($kalimat);
