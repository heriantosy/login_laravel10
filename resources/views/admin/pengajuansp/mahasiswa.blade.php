...
foreach($pengajuansp as $pengajuansp) {
  $i++;
  $NamaMhsx = strtolower($pengajuansp->NamaMhs);
  $NamaMhs	= ucwords($NamaMhsx);
  $JmlMK = DB::table('t_sp')->where('TahunID',$pengajuansp->TahunID)->where('MhswID',$pengajuansp->MhswID)->count();
?>
  <tr > 
    <td><?php echo $i ?></td>
    <td><?php echo $pengajuansp->MhswID ?></td>
    <td><a href="{{ asset('admin/pengajuansp/detailmk/'.$pengajuansp->MhswID.'/'.$tahunplh.'/'.$prodiplh) }}"><?php echo $NamaMhs ?></a> (<?php echo $JmlMK ?> Matakuliah)</td></td>
    <td><?php echo $pengajuansp->ProdiID ?></td>
  </tr>
<?php  
}
?>
