<form action="{{ asset('admin/formulir/proses') }}" method="post" accept-charset="utf-8">
...
<thead>
    ...
    <th width="20%">Jml Pilihan</th>
    <th width="10%">Aksi</th>
</tr>
</thead>
<tbody>

<?php $i=1; foreach($formulir as $formulir) { ?>

<tr class="odd gradeX">
    <td class="text-center">
      <div class="icheck-primary">
        <input type="checkbox" class="icheckbox_flat-blue " name="PMBFormulirID[]" value="<?php echo $formulir->PMBFormulirID ?>">
        <label for="check<?php echo $i ?>"></label>
      </div>
    </td>
    <td>{{ $formulir->PMBFormulirID }}</td>
    <td>{{ $formulir->Nama }}</td>
    <td>{{ $formulir->Harga }}</td>
    <td>{{ $formulir->JumlahPilihan }}</td>
  
    <td>
      <div class="btn-group">
        <a href="{{ asset('formulir/read/'.$formulir->PMBFormulirID) }}" 
        class="btn btn-success btn-sm" target="_blank"><i class="fa fa-eye"></i></a>
        <a href="{{ asset('admin/formulir/edit/'.$formulir->PMBFormulirID) }}" 
        class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a>

        <a href="{{ asset('admin/formulir/delete/'.$formulir->PMBFormulirID) }}" class="btn btn-danger btn-sm delete-link"><i class="fas fa-trash-alt"></i></a>
      </div>
    </td>
</tr>
